defmodule Scry.Engine.MongoDB.MixProject do
  use Mix.Project
  ...
  def project do
    [
      app: :scry_engine_mongodb_driver,
      version: @version,
      elixir: "~> 1.19",
  ...
  defp deps do
    [
      ...
      # === MONGODB DRIVER ===
      # `mongodb_driver` (distinct from the older, deprecated `mongodb`
      # package) -- actively maintained (v1.6.4, July 2026), DBConnection-
      # based, documents decode to plain string-keyed maps directly (no
      # decode step needed the way `boltx`'s typed `Node`/`Relationship`
      # structs needed for `scry_engine_neo4j`).
      {:mongodb_driver, "~> 1.6"},
  ...
  defp description do
    "A real Scry.Core.EngineBehaviour implementation over MongoDB, replacing scry_document's " <>
      "own in-memory reference Executor with genuine DEEP/PARENT/SIBLINGS/ANCESTORS execution " <>
      "against a real document store -- the document kind's first real adapter."
  end

  defp package do
    [
      licenses: ["MIT"],
      links: %{"GitHub" => "https://github.com/joetjen/scry_engine_mongodb_driver"},
  ...
  defp docs do
    [
      main: "readme",
      source_url: "https://github.com/joetjen/scry_engine_mongodb_driver",
